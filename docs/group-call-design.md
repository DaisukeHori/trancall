...
2. **サーバ側 heartbeat ハンドラ (`agent-routes.ts:487-573`) が翻訳セッション単位の仮計上行を upsert**:
   ...
   - **[v1.1.1 最終検証 W-A] 終了済みセッションの heartbeat 遅着ガード**: 解決した `translation_sessions` 行の **`ended_at IS NOT NULL` なら仮計上を行わない** (行は `findByAgentJobId` で既に取得済み・`endedAt` は parse 済みのため追加クエリゼロ、`translation-session-repository.supabase.ts:21`)。これにより「確定置換 (下記 4) や room ended 一括 DELETE (下記 6) の**後**に in-flight の heartbeat が処理されて仮計上行が復活する」経路を入口で遮断する。すり抜けるのは session_ended 処理と heartbeat 処理が同時進行するミリ秒級レースのみで、それは room ended 一括 DELETE と TTL (残余リスク対応) の受け持ちになる。
...
4. **[v1.1.1 Critical-2] 確定置換は settle-provisional-first — amount_yen 過大計上の防止**:
   ...
   - **[v1.1.1 最終検証 S-B] (a) 読み取りと (c) RPC DELETE の間の非原子性は許容**: この窓の間に遅着 heartbeat が仮計上値を bump しても、除外計算のずれは最大 1 heartbeat 間隔 (30 秒) の一時誤差に限定される (Agent は `end()` で heartbeat タイマーを先に停止するため窓は in-flight 分のみ、`translation-session.ts:476`)。枯渇境界近傍の数十秒の誤差として許容する (将来最適化: RPC 側 `DELETE ... RETURNING duration_seconds` で除外値を原子取得)。
...
5. **[W-3] 確定置換の原子性 — RPC (ストアド関数) の migration が必要**:
   ...
   - `UsageRepository` interface (`packages/billing/src/repositories/usage-repository.ts`) に以下を追加し、Supabase 実装 (`apps/server/src/adapters/repositories/billing/usage-repository.supabase.ts`) は `supabase.rpc(...)` で呼ぶ:
     ...
     - `upsertProvisionalWindow(...)` — heartbeat 仮計上 upsert (GREATEST ガード付き)。**[v1.1.1 最終検証 S-A] `ON CONFLICT ... DO UPDATE SET duration_seconds = GREATEST(...)` は supabase-js の `.upsert()` では表現できないため、こちらも SQL 関数 `upsert_provisional_window(...)` として同 migration に含め、`supabase.rpc(...)` で呼ぶ** (`findWindowByIdempotencyKey` / `deleteProvisionalByRoomId` は素の supabase-js で表現可能なので関数不要)
...
6. **[W-2] 仮計上行の「復活」防止 — room ended 時の一括 DELETE (PR1 スコープ)**:
   - **復活シナリオ**: 最終 `translation.ended` の確定置換 (DELETE) 後に、**in-flight の heartbeat** (置換前に送信済みで処理が遅延したもの) が仮計上行を再 INSERT すると、確定行と仮計上行が併存したまま以後の heartbeat が来ない (セッション終了済み) ため、**恒久過大計上**として固定化する。孤児は TTL 掃除まで `getUsedSecondsInPeriod` を二重計上させ、**次の通話の canStartCall / shouldContinue を誤って false にし得る**。
   - **一次対策 [W-A]**: 上記 2 の **`ended_at IS NOT NULL` ガード**が復活経路を入口で遮断する (確定置換時に `updateEnded` で `ended_at` が埋まるため、遅着 heartbeat は仮計上をスキップする)。
   - **二次対策**: room が `ended` へ遷移した時 (endCall / leave 自動終了 — reconcile と同じトリガーポイント、§6.4.3) に `deleteProvisionalByRoomId(roomId)` (`DELETE FROM usage_windows WHERE room_id = $1 AND is_provisional = true`) を実行する。**PR1 スコープ内の必須処理** — 旧 v1.1.0 が §13 の「TTL 掃除 (運用タスク)」へ送っていた掃除の一次責務を**撤回**し、設計内処理に格上げする。
   - 単調性ガード (上記 2 の GREATEST) は upsert 順序逆転による巻き戻りへの保険。復活防止は `ended_at` ガード (一次) → room ended 一括 DELETE (二次) → TTL (残余、§13) の三段構え — 残るのは session_ended 処理と heartbeat 処理のミリ秒級並行レースのみ。
...
| `usage_windows.is_provisional` + RPC `settle_usage_window` / `upsert_provisional_window` (D14) | 列追加 + ストアド関数 ×2 の migration | `DEFAULT false` で既存行は確定扱い。仮計上→確定置換は RPC で原子・冪等、GREATEST upsert も RPC (§6.2.3、W-3/S-A) |
...
3. **D14 の migration (`is_provisional` 列 + RPC `settle_usage_window` / `upsert_provisional_window`)**: migration を先に適用 → その後 server (heartbeat 仮計上 upsert / settle-provisional-first の recordUsage / room ended 一括 DELETE) をデプロイ → 最後に Agent (`translation-session.ts` の `elapsedBillableSeconds`・`languagePair` 送信)。旧 Agent は当該フィールド欠落のため仮計上がスキップされるだけで安全 (漸進有効化)。
...
| **[D14] 孤児仮計上行の TTL 掃除 (残余リスク対応のみ)** | **[v1.1.1 改訂] 一次の防止は §6.2.3-2 の `ended_at` ガード、掃除は §6.2.3-6 の room ended 一括 DELETE (PR1 スコープ) に格上げ済み** — 旧 v1.1.0 の「TTL 掃除 (運用タスク) 送り」を撤回 | ここに残るのは「room が `ended` に到達しない」異常系と `ended_at` ガードをすり抜けたミリ秒級並行レースのみ。保険として `recorded_at` が古い `is_provisional=true` 行の定期削除を運用タスクとする。通常は Agent 異常終了でも translation.ended が発行される想定 |
| **[D14/S-C] `session_started` POST の恒久失敗時は確定計上も漏れる (既存の best-effort 意味論)** | 記載のみ (v1.1.1 で悪化しない既存ギャップ) | `translation-session.ts:244-248` は warn のみで outbox 未実装。`session_started` が恒久失敗すると仮計上スキップに加え `updateEnded` も失敗し `translation.ended` 自体が publish されない (確定計上も漏れる)。仮計上の「スキップ→次 heartbeat 再試行」はこの既存意味論と整合。恒久対策 (outbox/リトライ) は別設計 |
...
| 日付 | バージョン | 変更内容 |
|---|---|---|
| 2026-07-17 | 1.1.2 | **v1.1.1 狭域最終検証 (修正後 GO、agentJobId 解決経路の妥当性は実コード一致・「1 job : N sessions」問題は agentJobId がセッションインスタンス毎採番 (`translation-session.ts:152`) のため不存在と確定) の残指摘を反映**。**W-A**: §6.2.3-2 に「解決した `translation_sessions` 行の `ended_at IS NOT NULL` なら仮計上を行わない」遅着ガードを追加 (追加クエリゼロ、`translation-session-repository.supabase.ts:21`) — 確定置換/一括 DELETE 後の in-flight heartbeat による仮計上行の復活を入口で遮断。§6.2.3-6 と §13 の「最終防壁/残るのは ended 未到達のみ」の過大主張を「`ended_at` ガード (一次) → room ended 一括 DELETE (二次) → TTL (残余) の三段構え、残余はミリ秒級並行レースのみ」に訂正。**S-A**: `upsert_provisional_window` SQL 関数 (GREATEST upsert は supabase-js の `.upsert()` で表現不可) を migration 内容 (§6.2.3-5/§11.1/§11.4/§12 PR1) に明記。**S-B**: settle-provisional-first の読み取り〜RPC DELETE 間の非原子性は最大 30 秒の一時誤差として許容と明記 (`translation-session.ts:476` のタイマー先行停止で窓は in-flight 分のみ)。**S-C**: `session_started` 恒久失敗時は確定計上ごと漏れる既存 best-effort 意味論 (outbox 未実装、`translation-session.ts:244-248`) を §13 に既知ギャップとして記載。 |
