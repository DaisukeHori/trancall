...
- 異常系: 同じプランへのアップグレードで `BILLING_INVALID_PLAN_CHANGE`
- 異常系: 一時的なネットワーク失敗で `BILLING_UPGRADE_PREVIEW_FAILED` (retryable=true)
...
**`startExternalPurchase`**:
- 正常系: `redirectUrl` が返る
- 異常系: Stripe API 失敗で `BILLING_PAYMENT_FAILED` (§8.4 と統一)

**`completeExternalPurchase`**:
- 正常系: 有効な `redirectToken` でサブスク更新
- 異常系: TTL 切れ `redirectToken` で `BILLING_PAYMENT_FAILED` (§8.4 と統一)
- 異常系: 使用済み `redirectToken` で `BILLING_PAYMENT_FAILED` (§8.4 と統一)
...
describe("BillingFacade integration", () => {
  ...
  describe("StoreKit External flow", () => {
    ...
    it("TTL 切れ redirectToken で BILLING_PAYMENT_FAILED が返る")
  })
...
| v1.2 | 2026-05-12 | Round 2 レビュー指摘 (C 不可判定) を反映。(C R1-M1 取りこぼし) §14.2 `previewUpgrade` 同一プラン case の error code を `BILLING_UPGRADE_PREVIEW_FAILED` から `BILLING_INVALID_PLAN_CHANGE` に修正、Round 1 の M-1 修正完遂。(C R2-W1) §14.2 `startExternalPurchase` 失敗 case を §8.4 と統一し `BILLING_PAYMENT_FAILED` に変更。(C R2-W2) §14.2 / §14.3 `completeExternalPurchase` 異常系 (TTL 切れ / 使用済み redirectToken) を §8.4 と統一し `BILLING_CHANNEL_NOT_AVAILABLE` から `BILLING_PAYMENT_FAILED` に変更。テスト仕様書全体で error code が §11.1 定義と整合。 |
