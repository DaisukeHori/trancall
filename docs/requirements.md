...
| ID | 要件 | 優先度 |
|----|------|--------|
| SCRIPT-001 | 通話中にリアルタイム字幕（原文 + 翻訳文）が表示される。原文転写は翻訳セッション内のinput transcriptを優先使用。品質不足の場合のみgpt-realtime-whisperを併用（追加$0.017/min） | Must |
...
|--------|------|-----------------|------------|
| Free | 0 yen | 5 min（お試し、m-009 確定） | 利用不可 |
...
| PERF-002 | 翻訳遅延（発話終了→翻訳音声開始） | p50 1.5s / p95 3.0s / p99 5.0s |
