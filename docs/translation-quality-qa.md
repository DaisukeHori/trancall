...
> **M-12 (2026-07 追記): マトリクス自動生成ツールの整備状況**
> §9.1/§9.2 のマトリクスは `apps/cron/quality-qa/matrix.ts` (`generateMatrixReport` /
> `pnpm --filter @trancall/cron quality-qa -- --generate-matrix <採点済みCSVパス>`) で
> §7.3 evaluator-sheet CSV (人手採点後、weighted_score/pass_fail/score_s が入力済みの状態) から
> **自動生成できる仕組みは整備済み**。同モジュールはシナリオ網羅チェック
> (`findCoverageGaps` / `checkFixtureDirectoryCoverage`) も提供し、未実施・未採点の
> (言語ペア, シナリオ) の組を機械的に列挙する。
>
> 一方で **§9.4 のサンプルマトリクス自体は依然として参考例のまま** であり、実際の
> 13(14)言語ペア × 5 シナリオの QA 判定 (実測値そのもの) は未実施である。これは
> LiveKit/OpenAI への実接続によるライブ収録 (§8.2、L-3 で自動トランスクリプト取得の
> 仕組みは整備済み) とネイティブ評価者によるスコアリング (§8.1.3、§8.3) が必要な
> **運用タスク**であり、コード実装だけでは完了できない。本ドキュメントは「実測を回す
> ための仕組み」が揃っていることを明記するに留め、実測値の記入は Sprint 3 の QA
> 実施後に行う。
