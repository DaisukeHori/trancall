...
| 項目 | 状態 | 詳細 |
|---|---|---|
| `android.googleServicesFile` | ⚠️ placeholder | `app.json` に `./google-services.json.PLACEHOLDER-REQUIRES-REAL-FIREBASE-CONFIG` (存在しないファイル名を意図的に指定、実配置時にエラーで気づけるようにする)。**実 Firebase プロジェクトの `google-services.json` を配置しないと実機ビルドは失敗する。** |
| `extra.eas.projectId` | ⚠️ placeholder | `REPLACE_WITH_REAL_EAS_PROJECT_ID_UUID`。実 EAS project 作成後に UUID へ差し替えが必要。 |
| `TRANCALL_PUSH_HMAC_SECRET` (EAS Secret) | ⚠️ 未設定 | `eas.json` の `development`/`preview`/`production` プロファイルに `env.TRANCALL_PUSH_HMAC_SECRET_SETUP_NOTE` として手順コメントを配置 (実値は書いていない)。実際の登録は `eas secret:create --scope project --name TRANCALL_PUSH_HMAC_SECRET --value <secret>` で行う (`docs/native-call-bridge.md` §12.1)。**登録するだけでは不十分**: Android 側は `BuildConfig.TRANCALL_PUSH_HMAC_SECRET` フィールド (gradle `buildConfigField`) への注入配線が必要だが、`android/app/build.gradle` が本リポジトリに実在しない (prebuild で生成される) ため未実装。iOS 側は `expo-secure-store` への書き込みタイミング・Keychain 読み出しロジックが `CallBridgeProvider.fetchHmacSecretPlaceholder()` に TODO として残っている。 |
...
| G-4 | Android: `google-services.json` 未配置 | FCM 自体が初期化できず `FcmService` が起動しない | 実 Firebase プロジェクトの設定ファイルを配置 |
| G-5 | Android: `BuildConfig.TRANCALL_PUSH_HMAC_SECRET` 注入未配線 | `FcmService.getHmacSecret()` が常に null → 着信が常に drop される | `android/app/build.gradle` (prebuild 生成後) に `buildConfigField` 追加 + EAS env 注入配線 |
