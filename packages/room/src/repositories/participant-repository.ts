...
export interface ParticipantRepository {
   ...
   */
  upsert(cmd: UpsertParticipantCommand): Promise<Result<ParticipantRow, AppError>>;

  /**
   * room_id に属する全参加者を取得する。
   */
  findByRoomId(roomId: RoomId): Promise<Result<ParticipantRow[], AppError>>;

  /**
   * まだ left_at が未設定の参加者全員の left_at を now に更新する。
   */
  setLeftAtForAll(roomId: RoomId, leftAt: string): Promise<Result<true, AppError>>;
}
