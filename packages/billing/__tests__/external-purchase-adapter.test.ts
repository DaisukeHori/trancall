...
describe("ExternalPurchaseAdapter.startExternalPurchase", () => {
  it("正常系: redirectUrl と redirectToken を返す", async () => {
    ...
    const result = await adapter.startExternalPurchase(
      userId,
      "standard",
    ...
    expect(result.data.redirectToken).toHaveLength(64);
  });
});
